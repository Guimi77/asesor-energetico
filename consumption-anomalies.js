...
(function(root,factory){
  ...
  function duration(r){
    const stated=number(r?.billing_days);
    if(stated!=null){const d=Math.round(stated);return Math.abs(stated-d)<0.001&&d>=21&&d<=40?d:null;}
    const a=day(r?.billing_start),b=day(r?.billing_end);if(a==null||b==null||b<=a)return null;
    const d=b-a;return d>=21&&d<=40?d:null;
  }
  ...
  function ref(r){const d=duration(r),kwh=number(r.consumption_kwh);return {id:r.id,invoice:r.invoice_number||'Sin referencia',start:r.billing_start,end:r.billing_end,days:d,kwh,kwhDay:kwh/d,readingStatus:String(r.reading_status||'unknown')};}
  ...
  function analyze({records=[],supplies=[]}={}){
    ...
    for(const [supplyId,source] of groups){
      ...
      const baseValues=baseDaily.map(v=>format(v,2)).join(', '),recentValues=recentDaily.map(v=>format(v,2)).join(' y ');
      items.push({
        ...
        evidence:`Referencia: mediana de los 3 periodos anteriores (${baseValues} kWh/día) = ${format(base)} kWh/día. Periodos recientes: ${recentValues} kWh/día; su media es ${format(recentAvg)} kWh/día. La diferencia frente a la referencia es ${up?'+':'-'}${format(pct,1)} %. El cálculo usa únicamente kWh y días de facturación guardados de facturas validadas; no estima consumos que no estén en las facturas.`,
        ...
        baselineValuesKwhDay:baseDaily,recentValuesKwhDay:recentDaily,
  ...
  function render({records=[],supplies=[],holders=[],signalItems=null}={}){
    ...
    const cards=items.map(item=>{
      const s=supplyMap.get(item.supplyId),h=holderMap.get(s?.holder_id),name=s?.supply_name||'Suministro eléctrico',identity=[h?.legal_name,s?.cups].filter(Boolean).join(' · ');
      const rows=item.sources.map((r,i)=>`<tr><td>${i<RULES.baselineRecords?'Referencia':'Reciente'}</td><td>${esc(r.invoice)}</td><td>${esc(String(r.start||'').split('-').reverse().join('/'))} – ${esc(String(r.end||'').split('-').reverse().join('/'))}</td><td>${r.days??'—'}</td><td>${format(r.kwh,2)} kWh</td><td>${format(r.kwhDay,2)} kWh/día</td><td>${r.readingStatus==='actual'?'Real confirmada':'No determinada'}</td></tr>`).join('');
      return `<details class="history-rec"><summary><span><strong>${esc(item.title)}</strong><span class="history-rec-name">${esc(name)}</span><span class="history-scope">${esc(identity)}</span></span><span class="history-rec-status">Confianza ${esc(item.confidence)}<span>${item.changeRatio>0?'+':''}${format(item.changeRatio*100,1)} %</span></span></summary><div class="history-rec-body"><h4>Qué cambio se ha detectado</h4><p>${esc(item.evidence)}</p><div class="history-table-wrap"><table class="history-mini-table"><thead><tr><th>Grupo</th><th>Factura</th><th>Periodo</th><th>Días</th><th>Consumo</th><th>Consumo diario</th><th>Lectura</th></tr></thead><tbody>${rows}</tbody></table></div><h4>Qué proponemos revisar</h4><p>${esc(item.action)}</p><p class="history-rec-caution">${esc(item.caveat)} No es una estimación de ahorro.</p></div></details>`;
    }).join('');
    return `<section class="card history-recommendations" id="historyConsumptionChanges" aria-labelledby="historyConsumptionChangesTitle"><div class="history-section-head"><div><p class="eyebrow">Comportamiento del consumo · señal de revisión</p><h2 id="historyConsumptionChangesTitle">Cambios sostenidos de consumo</h2></div><span class="history-pill">${items.length} señal(es)</span></div><p class="history-scope">Comparamos kWh/día usando los días de facturación indicados en cada factura. Exigimos 5 periodos consecutivos: 3 como referencia y 2 recientes en la misma dirección. Las lecturas estimadas, la ausencia de lectura de distribuidora y los 0 kWh sin lectura real confirmada no forman parte de esta comparación.</p><div class="history-rec-list">${cards}</div></section>`;
  }
