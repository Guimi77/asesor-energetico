...
const PARSER_VERSION='2026.09.22.1';window.IBT_PARSER_VERSION=PARSER_VERSION;
...
async function parseInvoiceFile(file){
 ...
 if(window.IBTIberdrolaParser?.detect?.(original)||window.IBTRepsolParser?.detect?.(original)||window.IBTNaturgyParser?.detect?.(original))return parseInvoice(original,file);
