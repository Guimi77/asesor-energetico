(() => {
  ...
  function renderSearchMenu(input, menu, items, label, activeId = '', { allLabel = '', showAll = false } = {}) {
    ...
    const allOption = allLabel
      ? `<button type="button" class="history-combo-option ${activeId ? '' : 'active'}" role="option" data-label="">${esc(allLabel)}</button>`
      : '';
    const options = filtered.slice(0, 250).map(item => {
      const text = label(item);
      const active = item.id === activeId ? ' active' : '';
      return `<button type="button" class="history-combo-option${active}" role="option" data-label="${esc(text)}">${esc(text)}</button>`;
    }).join('');
        ...
        <div id="historyApp" class="history-app">
          <section class="card history-controls-card">
            <div class="history-toolbar">
              ...
              <label>Titular<div class="history-combo"><input id="historyHolderSearch" class="history-search-input" autocomplete="off" role="combobox" aria-autocomplete="list" aria-controls="historyHolderMenu" aria-expanded="false" placeholder="Todos los titulares · escribe para buscar"><button id="historyHolderToggle" class="history-combo-toggle" type="button" aria-label="Abrir lista de titulares" title="Abrir lista">▾</button><div id="historyHolderMenu" class="history-combo-menu" role="listbox" hidden></div></div></label>
              <label>CUPS<div class="history-combo"><input id="historySupplySearch" class="history-search-input" autocomplete="off" role="combobox" aria-autocomplete="list" aria-controls="historySupplyMenu" aria-expanded="false" placeholder="Todos los CUPS · escribe para buscar"><button id="historySupplyToggle" class="history-combo-toggle" type="button" aria-label="Abrir lista de CUPS" title="Abrir lista">▾</button><div id="historySupplyMenu" class="history-combo-menu" role="listbox" hidden></div></div></label>
  ...
  function renderClientControl() {
    ...
    if (internal) {
      ...
      host.innerHTML = `<div class="history-combo"><input id="historyClientSearch" class="history-search-input" autocomplete="off" role="combobox" aria-autocomplete="list" aria-controls="historyClientMenu" aria-expanded="false" value="${esc(current?.name || '')}" placeholder="Escribe para buscar cliente"><button id="historyClientToggle" class="history-combo-toggle" type="button" aria-label="Abrir lista de clientes" title="Abrir lista">▾</button><div id="historyClientMenu" class="history-combo-menu" role="listbox" hidden></div></div>`;
      const input = $('#historyClientSearch');
      const menu = $('#historyClientMenu');
      const openMenu = (showAll = false) => renderSearchMenu(input, menu, state.clients, c => c.name, state.currentClient, { showAll });
      input?.addEventListener('input', () => { markSearch(input, true); openMenu(false); });
      input?.addEventListener('change', () => { closeSearchMenu(input, menu); void applyClientSearch(input); });
      ...
      $('#historyClientToggle')?.addEventListener('click', e => {
        e.preventDefault();
        input?.focus();
        openMenu(true);
      });
  ...
  function bindFilters() {
    ...
    $('#historyHolderToggle')?.addEventListener('click', e => {
      e.preventDefault();
      holder?.focus();
      openHolderMenu(true);
    });
    holderMenu?.addEventListener('pointerdown', e => {
      const option = e.target.closest('.history-combo-option');
      if (!option) return;
      e.preventDefault();
    ...
    $('#historySupplyToggle')?.addEventListener('click', e => {
      e.preventDefault();
      supply?.focus();
      openSupplyMenu(true);
    });
    supplyMenu?.addEventListener('pointerdown', e => {
      const option = e.target.closest('.history-combo-option');
      if (!option) return;
      e.preventDefault();
    ...
    document.addEventListener('pointerdown', e => {
      if (!e.target.closest('.history-combo')) closeAllSearchMenus();
    });
