(() => {
  ...
  function detectedEvents(records) {
    ...
    for (const [supplyId, list] of bySupply) {
      ...
      for (let i=1;i<list.length;i++) {
        ...
        if ((prev.tariff||'') !== (cur.tariff||'')) events.push({date:cur.billing_start,type:'Cambio de tarifa',supplyId,text:`${prev.tariff||'—'} → ${cur.tariff||'—'}`});
        const p0=powerSignature(prev), p1=powerSignature(cur);
        if (p0 && p1 && p0!==p1) events.push({date:cur.billing_start,type:'Cambio de potencia',supplyId,text:`${describePower(prev)} → ${describePower(cur)}`});
      }
    }
  ...
  function render(records) {
    ...
    host.innerHTML = `
      ...
      <section class="card">
        <div class="history-section-head"><div><p class="eyebrow">Cambios detectados</p><h2 style="margin:0">Evolución contractual</h2></div><span class="history-scope">Tarifa y potencia contratada</span></div>
        <div class="history-events">${events.length ? events.map(e=>{const s=supplyById(e.supplyId);return `<div class="history-event"><span>${dateES(e.date)}</span><b>${esc(e.type)}</b><span>${esc(s?.cups||'')} · ${esc(e.text)}</span></div>`}).join('') : '<div class="history-empty">Todavía no hay suficientes periodos consecutivos para detectar cambios de tarifa o potencia.</div>'}</div>
      </section>`;
