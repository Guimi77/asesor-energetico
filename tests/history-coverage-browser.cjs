...
(async()=>{
 ...
 try {
  ...
  const coverageText=await page.locator('.history-coverage').innerText();
  assert.match(coverageText,/Hay meses con pocos datos/);
  assert.match(coverageText,/no dibujamos 1 mes/);
  ...
  assert.equal(await page.locator('[data-coverage-month]').count(),1);
  ...
  assert.match(await page.locator('.history-coverage').innerText(),/Hay meses sin datos/);
  ...
  assert.equal(await page.locator('[data-coverage-month]').count(),0);assert.equal(await page.locator('svg[data-field="kwh"] circle').count(),1);
