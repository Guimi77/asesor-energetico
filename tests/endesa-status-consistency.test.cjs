...
const baseRow=overrides=>({
  sourceFormat:'endesa',unsupported:false,company:'EMPRESA PRUEBA ZETA, SL',cups:'ES0000000000000006AA0F',
  ...
  supplyAddress:'C/ EJEMPLO PRUEBA 17 1, 07012 PALMA, Referencia del contrato de acceso: 500000000004',
  ...overrides
});

const mallorcaDoc={pages:[
  ['Potencia 104,35 €','Energía 197,90 €','Impuestos 82,70 €','Total 389,37 €'],
  ['Dirección de suministro: C/ EJEMPLO PRUEBA 17 1, 07012 PALMA,','BALEARES','Referencia del contrato de acceso: 500000000004',
...
test('A coherent 3.0TD row is correct even when PDF power detail is fragmented',()=>{
  ...
  assert.equal(r.supplyAddress,'C/ EJEMPLO PRUEBA 17 1, 07012 PALMA, BALEARES');
...
test('Estimated 3.0TD readings are correct when source billing evidence is coherent',()=>{
  const row=baseRow({
    company:'CLIENTE PRUEBA ALFA',cups:'ES0000000000000001AA0F',period:'11/05/2026 - 04/06/2026 (24 días)',
  ...
  const d={pages:[
    ...
    ['Dirección de suministro: C/ EJEMPLO PRUEBA 1, 07191 BANYALBUFAR,','BALEARES',
...
test('Address cleanup stops before Endesa contract metadata',()=>{
  const rafael=['Dirección de suministro: C/ EJEMPLO PRUEBA 45 2 ,, 07190','ESPORLES, BALEARES','Referencia del contrato de acceso: 500000000005','Peaje de transporte y distribución: 2.0TD'];
  const miguel=['Dirección de suministro: C/ EJEMPLO PRUEBA 19 BJO,','07340 ALARO, BALEARES','Referencia del contrato de acceso: 500000000002','Peaje de transporte y distribución: 2.0TD'];
  assert.equal(guard.sourceAddress(rafael,''),'C/ EJEMPLO PRUEBA 45 2, 07190, ESPORLES, BALEARES');
  assert.deepEqual(guard.placeFromAddress(guard.sourceAddress(rafael,'')),{city:'ESPORLES',province:'BALEARES'});
  assert.equal(guard.sourceAddress(miguel,''),'C/ EJEMPLO PRUEBA 19 BJO, 07340 ALARO, BALEARES');
