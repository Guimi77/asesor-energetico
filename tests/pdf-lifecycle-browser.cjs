 ...
 try{
  ...
  const summary=await tab.evaluate(async bytes=>{
   ...
   for(let i=0;i<341;i++){
    ...
    if(a.text!==b.text||a.pages.length!==2||b.pages.length!==2||!c.read){
      throw Error('Reader output changed: '+JSON.stringify({mainText:a.text,historyText:b.text,mainPages:a.pages.length,historyPages:b.pages.length,masterRead:c.read}));
    }
