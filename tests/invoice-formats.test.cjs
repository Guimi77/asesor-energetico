...
test('Endesa two-column 2.0TD text keeps every reading instead of losing right-column periods',()=>{
  ...
  const p2=['Titular del contrato: RICARD JORDA MARCOS ....................................................... CUPS: ES0000000000000000000F','Potencias contratadas: punta-llano 5,750 kW; valle 5,750 kW','Peaje de transporte y distribución: 2.0TD','Punta 24.606,217 24.849,430 1,00 0,000 243,213','texto izquierdo Llano 7.737,018 7.974,459 1,00 0,000 237,441','otro texto Valle 9.337,632 9.712,513 1,00 0,000 374,881','Pot. Punta-Llano 5,750 kW x 0,102310 Eur/kW x 32 días 18,82 €','Pot. Valle 5,750 kW x 0,017763 Eur/kW x 32 días 3,27 €','Impuesto electricidad ( 206,76 Eur X 5,1126963 %) 10,57 €','IVA normal 21 % s/ 218,18 45,82 €'];
  const r=api.parseEndesa(d(p1,p2),{name:'merged2.pdf'});
  assert.equal(r.company,'RICARD JORDA MARCOS');assert.equal(r.periods.P1.consumption,243.213);assert.equal(r.periods.P2.consumption,237.441);assert.equal(r.periods.P3.consumption,374.881);assert.equal(r.readOk,true);
});
...
test('Endesa two-column 3.0TD text uses exact active-energy register codes',()=>{
  ...
  const p2=['Titular del contrato: ELSEBETH SVENDSEN ............................................ Número de contador: 300056014','Potencia contratada [kW]: P1 15,010; P2 15,010; P3 15,010; P4 15,010; P5 15,010; P6 15,010.','CUPS: ES0000000000000000001F','Peaje de transporte y distribución: 3.0TD','ENERGÍA ACTIVA kWh P3 0 1,00 0,000','P1 1.18.1 12.028,667 12.206,986 1,00 0,000 178,319 P4 0 0,00 0,000','P2 1.18.2 19.046,997 19.809,805 1,00 0,000 762,808 P5 0 0,00 0,000','P3 1.18.3 17.732,082 18.217,505 1,00 0,000 485,423 P6 0 1,00 0,000','P4 1.18.4 16.794,474 16.794,474 1,00 0,000 0,000','P5 1.18.5 10.562,016 10.562,016 1,00 0,000 0,000','P6 1.18.6 59.047,670 60.474,220 1,00 0,000 1.426,550','P1 1.16.1 5,628 1 5,628','P2 1.16.2 6,067 1 6,067','P3 1.16.3 5,795 1 5,795','P4 1.16.4 0,000 1 0,000','P5 1.16.5 0,000 1 0,000','P6 1.16.6 6,873 1 6,873','Pot. P1 15,010 kW x 24 días x 0,067393 Eur/kW y día 24,28 €','Pot. P2 15,010 kW x 24 días x 0,034203 Eur/kW y día 12,32 €','Pot. P3 15,010 kW x 24 días x 0,014995 Eur/kW y día 5,40 €','Pot. P4 15,010 kW x 24 días x 0,013083 Eur/kW y día 4,71 €','Pot. P5 15,010 kW x 24 días x 0,008671 Eur/kW y día 3,12 €','Pot. P6 15,010 kW x 24 días x 0,006407 Eur/kW y día 2,31 €','Impuesto Electricidad 507,13 Eur x 5,1126963 % 25,93 €','IVA normal (21%) 21 % s/ 541,58 113,73 €'];
  const r=api.parseEndesa(d(p1,p2),{name:'merged3.pdf'});
  assert.equal(r.company,'ELSEBETH SVENDSEN');assert.equal(r.periods.P1.consumption,178.319);assert.equal(r.periods.P2.consumption,762.808);assert.equal(r.periods.P3.consumption,485.423);assert.equal(r.periods.P6.consumption,1426.55);assert.equal(r.maximeters.P6,6.873);assert.equal(r.readOk,true);
});

test('Older Endesa layout accepts a-period wording and keeps P3',()=>{
  const p1=['Nº factura: P24CON018734946','Periodo de facturación: del 07/04/2024 a 11/05/2024 (34 días)','Potencia 17,36 €','Energía 76,57 €','Descuentos -22,98 €','Otros 1,11 €','Impuestos 18,40 €','Total 90,46 €','del Consumo Total 305,499 kWh'];
  const p2=['Titular del contrato: MIGUEL ANGEL MUNAR HOMAR ....................................................... CUPS: ES0031500598524004EW0F','Potencias contratadas: punta 5,750 kW; valle 5,750 kW','Peaje de transporte y distribución: 2.0TD','Punta 18.820,79 18.919,15 1,00 0,00 98,36','Llano 2.410,66 2.475,15 1,00 0,00 64,49','texto Valle 4.862,89 5.005,53 1,00 0,00 142,65','Pot.Punta 5,750 kW x 0,085838 Eur/kW x 34 días 16,78 €','Pot. Valle 5,750 kW x 0,002989 Eur/kW x 34 días 0,58 €','Impuesto electricidad ( 71,16 Eur X 3,8 %) 2,70 €','IVA normal 21 % s/ 74,76 15,70 €'];
...
test('Endesa refuses a green status when period consumption does not reconcile',()=>{
  ...
  const p2=['Titular del contrato: TEST','Potencias contratadas: punta 3,450 kW; valle 3,450 kW','CUPS: ES0000000000000000003F','Peaje de transporte y distribución: 2.0TD','Punta 1,00 2,00 1,00 0,00 100,00','Llano 1,00 2,00 1,00 0,00 50,00','Valle 1,00 2,00 1,00 0,00 50,00','Pot. Punta 3,450 kW x 31 días x 0,08 Eur/kW x día 8,00 €','Pot. Valle 3,450 kW x 31 días x 0,02 Eur/kW x día 2,00 €','Impuesto electricidad 30,00 Eur x 5 % 1,50 €','IVA normal 21 % s/ 31,50 4,80 €'];
