...
(async()=>{
 ...
 try{
  ...
  await page.fill('#historyFrom','2027-01-01');await page.locator('#historyFrom').dispatchEvent('change');await page.waitForSelector('#historyCostChart .history-cost-missing');assert.equal(await page.locator('#historyCostChart circle').count(),0);assert.equal(await page.locator('.history-grid>.history-chart').count(),3);
  await page.goto(root+'/candidate/?ready=1&case=zero');await page.waitForSelector('#historyCostChart .history-cost-missing');assert.equal(await page.locator('#historyCostChart circle').count(),0);assert.equal(await page.locator('#historyCostChart path').count(),0);assert.equal(await page.locator('#historyCostChart .history-cost-missing').count(),2);
