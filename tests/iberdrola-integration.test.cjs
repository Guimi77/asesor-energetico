...
test('main parser routes Iberdrola before legacy formats and before FENIE OCR fallback',()=>{
  ...
  const fileRoute=app.indexOf('if(window.IBTIberdrolaParser?.detect?.(original)||window.IBTRepsolParser?.detect?.(original)||window.IBTNaturgyParser?.detect?.(original))return parseInvoice(original,file)');
