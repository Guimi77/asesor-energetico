...
test('Old Endesa layout recovers wrapped period and total even when PDF.js prefixes dots',()=>{
  ...
  const p2=[
    'Titular del contrato: CLIENTE PRUEBA BETA','NIF: 00000002B',
    'Dirección de suministro: C/ EJEMPLO PRUEBA 19 BJO,','07340 ALARO, BALEARES',
    'Contrato de mercado libre: Libre Endesa','Referencia de contrato de suministro: 600000000002',
    'Potencias contratadas: punta 5,750 kW; valle 5,750 kW','CUPS: ES0000000000000002AA0F','Distribuidora: EDISTRIBUCION REDES DIGITALES','Referencia del contrato de acceso: 500000000002','Peaje de transporte y distribución: 2.0TD',
  ...
  const r=api.parseEndesa(doc(p1,p2),{name:'cliente-prueba-beta.pdf'});
  ...
  assert.equal(r.supplyAddress,'C/ EJEMPLO PRUEBA 19 BJO, 07340 ALARO, BALEARES');
  assert.equal(r.supplyCity,'ALARO');
  assert.equal(r.supplyProvince,'BALEARES');
  assert.equal(r.contract,'600000000002');
  assert.equal(r.accessContract,'500000000002');
});
...
test('Reactive measurements with zero billed amount stay zero and all six contracted periods are preserved',()=>{
  ...
  const p1=[
    'Endesa Energía, S.A. Unipersonal.','IMPORTE FACTURA: 655,31 €','Nº de factura: P26CON000000001','Periodo de facturación: del 11/05/2026 al 04/06/2026 (24 días)',
    'Potencia 52,14 €','Energía 525,89 €','Descuentos -71,36 €','Otros 8,98 €','Impuestos 139,66 €','Total 655,31 €','Consumo Total 2.853,101 kWh'
  ];
  const p2=[
    'Titular del contrato: CLIENTE PRUEBA ALFA','NIF: X0000000A','Dirección de suministro: C/ EJEMPLO PRUEBA 1, 07191 BANYALBUFAR,','BALEARES','Contrato de mercado libre: Tempo Open',
    'Potencia contratada [kW]: P1 15,010; P2 15,010; P3 15,010; P4 15,010;','P5 15,010; P6 15,010.','CUPS: ES0000000000000001AA0F','Referencia del contrato: 60000000001','Referencia del contrato de acceso: 500000000001','Peaje de transporte y distribución: 3.0TD',
  ...
  const r=api.parseEndesa(doc(p1,p2),{name:'cliente-prueba-alfa.pdf'});
  ...
  assert.ok(learned);assert.equal(learned.cups,'ES0000000000000001AA0F');assert.equal(learned.contract,'60000000001');assert.equal(learned.supplyAddress,'C/ EJEMPLO PRUEBA 1, 07191 BANYALBUFAR, BALEARES');
});

test('Explicit Endesa power summary stays valid when PDF line fragmentation hides the per-period power breakdown',()=>{
  const raw={
    company:'CLIENTE PRUEBA ALFA',cups:'ES0000000000000001AA0F',period:'11/05/2026 - 04/06/2026 (24 días)',tariff:'3.0TD',
    ...
    serviceTotal:0,supplyAddress:'C/ EJEMPLO PRUEBA 1, 07191 BANYALBUFAR, BALEARES',contract:'60000000001',accessContract:'500000000001'
  };
  ...
  const p2=['Dirección de suministro: C/ EJEMPLO PRUEBA 1, 07191 BANYALBUFAR,','BALEARES','Potencia contratada [kW]: P1 15,010; P2 15,010; P3 15,010; P4 15,010; P5 15,010; P6 15,010.','EXCESOS DE POTENCIA kW','Periodo horario Contratada Demandada A facturar','P1 15,010 5,628 0,000','P2 15,010 6,067 0,000','P3 15,010 5,795 0,000','P4 15,010 0,000 0,000','P5 15,010 0,000 0,000','P6 15,010 6,873 0,000','ENERGÍA REACTIVA INDUCTIVA kWh','Periodo horario Consumo Cos A facturar','P1 85,000 1,00 0,000','P2 210,000 1,00 0,000','P3 104,000 1,00 0,000','P4 29,000 0,00 0,000','P5 82,000 0,00 0,000','P6 91,000 1,00 0,000'];
...
test('Catalan Endesa 2.0TD layout is normalized before validation',()=>{
  const api=fix.patch(base,{});
  const p1=[
    'Endesa Energia, S.A. Unipersonal.','Núm. factura: P26CON000000007','Període de facturació: del 19/07/2026 a 19/08/2026 (31 dies)',
    'Potència 31,32 €','Energia 101,71 €','Altres 1,60 €','Impostos 36,55 €','Total 171,18 €','Consum Total 736,563 kWh'
  ];
  const p2=[
    'Titular del contracte: CLIENTE PRUEBA GAMMA','NIF: 00000003C','Adreça de subministrament: C/ EJEMPLO PRUEBA 29 1,','07190 ESPORLES, LES BALEARS',
    'Contracte de mercat lliure: One 3 Períodes','Referència de contracte de subministrament: 600000000003','Referència del contracte d\'accés: 500000000003',
    'Potències contractades: punta-pla 5,600 kW; vall 5,600 kW','CUPS: ES0000000000000003AA0F','Peatge de transport i distribució: 2.0TD',
  ...
  assert.equal(r.company,'CLIENTE PRUEBA GAMMA');assert.equal(r.period,'19/07/2026 - 19/08/2026 (31 días)');
