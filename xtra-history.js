...
function extractHistory(d,file){const iberdrola=window.IBTIberdrolaParser;if(iberdrola?.detect?.(d))return extractIberdrola(d,file);const repsol=window.IBTRepsolParser;if(repsol?.detect?.(d))return extractRepsol(d,file);const naturgy=window.IBTNaturgyParser;if(naturgy?.detect?.(d))return extractNaturgy(d,file);const formats=window.IBTInvoiceFormats,format=formats?.detect?.(d.text);if(format==='endesa')return extractEndesa(d,file);if(format==='fenie')return extractFenie(d,file);return null;}
...
const source=await readPdf(file),iberdrola=window.IBTIberdrolaParser,repsol=window.IBTRepsolParser,naturgy=window.IBTNaturgyParser,fallback=window.IBTFenieOcrFallback,prepared=(iberdrola?.detect?.(source)||repsol?.detect?.(source)||naturgy?.detect?.(source))?{data:source,attempted:false,error:null}:fallback?.prepare?await fallback.prepare(file,source,pdfjsLib):{data:source,attempted:false,error:null};
