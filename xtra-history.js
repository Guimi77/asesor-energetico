...
const periodText=norm(row.period),period=parsePeriod(periodText),tariff=norm(row.tariff),periods=row.periods||{},contracted=row.contracted||{},mx=row.maximeters||{};
const energyPeriods=Object.entries(periods).filter(([key])=>/^P[1-6]$/.test(key)).map(([key,value])=>({period:Number(key.slice(1)),consumption_kwh:value?.consumption==null?null:Number(value.consumption),energy_cost_eur:value?.cost==null?null:Number(value.cost),unit_price_eur_kwh:value?.price==null?null:Number(value.price)}));
const entries=row.powerDetail?.entries||[],powerKeys=Object.keys(contracted).filter(key=>/^P[1-6]$/.test(key)).sort((a,b)=>Number(a.slice(1))-Number(b.slice(1)));
const powerPeriods=powerKeys.map(key=>{const periodNo=Number(key.slice(1)),entry=entries.find(item=>Number(item?.period)===periodNo)||entries[periodNo-1];return{period:periodNo,contracted_kw:Number(contracted[key])||0,billed_power_eur:entry?.amount==null?null:Number(entry.amount),unit_price_eur_kw_day:entry?.price==null?null:Number(entry.price)};});
const maximeterRows=Object.keys(mx).filter(key=>/^P[1-6]$/.test(key)).map(key=>({period:Number(key.slice(1)),maximeter_kw:Number(mx[key])||0,reliable:!!mx._reliable,source:'IBERDROLA · tabla maxímetro'}));
const taxLines=[];if(Number(row.tax))taxLines.push({tax_type:'ELECTRICITY_TAX',label:'Impuesto electricidad',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.tax)});if(Number(row.vat))taxLines.push({tax_type:'IVA',label:'IVA',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.vat)});if(Number(row.igic))taxLines.push({tax_type:'IGIC',label:'IGIC',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.igic)});
const adjustments=[];if(Number(row.discounts))adjustments.push({concept:'Descuento sobre consumo',amount_eur:Number(row.discounts),category:'discount'});
const reactiveMeasurements=row.reactivePeriods||{};
const reactivePeriods=Object.entries(reactiveMeasurements)
  .filter(([key,value])=>/^P[1-6]$/.test(key)&&value!=null&&Number.isFinite(Number(value)))
  .map(([key,value])=>({period:Number(key.slice(1)),reactive_kvarh:Number(value),consumption_kvarh:Number(value),amount_eur:0,cos_phi:null,excess_kvarh:null,unit_price_eur_kvarh:null}));
const capacitiveMeasurements=row.capacitivePeriods||{};
const energyPriceComplete=energyPeriods.length>0&&energyPeriods.every(item=>item.consumption_kwh===0||item.unit_price_eur_kwh!=null),powerPriceComplete=powerPeriods.length>0&&powerPeriods.every(item=>item.unit_price_eur_kw_day!=null);
const completeness={version:COMPLETENESS_VERSION,invoice_number:status(row.invoiceNumber),cups:status(row.cups),billing_period:status(period.start&&period.end),issue_date:status(row.issueDate,false),holder_name:status(row.company),holder_tax_id:status(row.taxId,false),supply_address:status(row.supplyAddress),access_contract_number:status(row.accessContract,false),contract_number:status(row.contract,false),contract_type:status(row.contractType,false),contract_end_date:status(row.renewalDate,false),meter_number:status(row.meterNumber,false),tariff:status(tariff),distributor:status(row.distributor,false),energy_periods:energyPeriods.length?'extracted':'unreliable',energy_price_components:energyPriceComplete?'extracted':'unreliable',power_periods:row.powerDetail?.reliable&&powerPeriods.length?'extracted':'unreliable',power_price_components:powerPriceComplete?'extracted':'unreliable',maximeters:mx._reliable?'extracted':'unreliable',excess_detail:Number(row.excess)?'extracted':'not_present',reactive_detail:Number(row.reactive)?'extracted':'not_present',reactive_measurements:reactivePeriods.length?'extracted':'not_present',capacitive_measurements:Object.keys(capacitiveMeasurements).length?'extracted':'not_present',compensation:Number(row.compensation)?'extracted':'not_present',social_bonus:Number(row.social)?'extracted':'not_present',meter_rental:Number(row.rental)?'extracted':'not_present',electricity_tax:Number(row.tax)?'extracted':'not_present',tax_lines:taxLines.length?'extracted':'not_present',distributor_rights:'not_present',integrator_adjustment:'not_present',reactive_regularization:'not_present'};
const assessment=row.readOk&&energyPeriods.length&&powerPeriods.length?'complete':'needs_review';
...
function extractRepsol(d,file){
const parser=window.IBTRepsolParser;if(!parser?.parse)return null;
const row=parser.parse(d,file,{parserVersion:window.IBT_PARSER_VERSION||'REPSOL',readingClassifier:window.IBTReadingStatus?.classify});if(!row||row.unsupported)return null;
const periodText=norm(row.period),period=parsePeriod(periodText),tariff=norm(row.tariff),periods=row.periods||{},contracted=row.contracted||{},mx=row.maximeters||{};
const energyPeriods=Object.entries(periods).filter(([key])=>/^P[1-6]$/.test(key)).map(([key,value])=>({period:Number(key.slice(1)),consumption_kwh:value?.consumption==null?null:Number(value.consumption),energy_cost_eur:value?.cost==null?null:Number(value.cost),unit_price_eur_kwh:value?.price==null?null:Number(value.price)}));
const entries=row.powerDetail?.entries||[],powerKeys=Object.keys(contracted).filter(key=>/^P[1-6]$/.test(key)).sort((a,b)=>Number(a.slice(1))-Number(b.slice(1)));
const powerPeriods=powerKeys.map(key=>{const periodNo=Number(key.slice(1)),entry=entries.find(item=>Number(item?.period)===periodNo)||entries[periodNo-1];return{period:periodNo,contracted_kw:Number(contracted[key])||0,billed_power_eur:entry?.amount==null?null:Number(entry.amount),unit_price_eur_kw_day:entry?.price==null?null:Number(entry.price)};});
const maximeterRows=Object.keys(mx).filter(key=>/^P[1-6]$/.test(key)).map(key=>({period:Number(key.slice(1)),maximeter_kw:Number(mx[key])||0,reliable:!!mx._reliable,source:'REPSOL · máxima demandada anual'}));
const taxLines=[];if(Number(row.tax))taxLines.push({tax_type:'ELECTRICITY_TAX',label:'Impuesto electricidad',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.tax)});if(Number(row.vat))taxLines.push({tax_type:'IVA',label:'IVA',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.vat)});if(Number(row.igic))taxLines.push({tax_type:'IGIC',label:'IGIC',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.igic)});
const adjustments=Array.isArray(row.adjustments)?row.adjustments.map(item=>({...item})):[];if(!adjustments.length&&Number(row.discounts))adjustments.push({concept:'Descuento comercial',amount_eur:Number(row.discounts),category:'discount'});
const reactiveMeasurements=row.reactivePeriods||{};
const reactivePeriods=Object.entries(reactiveMeasurements)
  .filter(([key,value])=>/^P[1-6]$/.test(key)&&value!=null&&Number.isFinite(Number(value)))
  .map(([key,value])=>({period:Number(key.slice(1)),reactive_kvarh:Number(value),consumption_kvarh:Number(value),amount_eur:0,cos_phi:null,excess_kvarh:null,unit_price_eur_kvarh:null}));
...
const completeness={version:COMPLETENESS_VERSION,invoice_number:status(row.invoiceNumber),cups:status(row.cups),billing_period:status(period.start&&period.end),issue_date:status(row.issueDate,false),holder_name:status(row.company),holder_tax_id:status(row.taxId,false),supply_address:status(row.supplyAddress),access_contract_number:status(row.accessContract,false),contract_number:status(row.contract,false),contract_type:status(row.contractType,false),contract_end_date:status(row.renewalDate,false),meter_number:status(row.meterNumber,false),tariff:status(tariff),distributor:status(row.distributor,false),energy_periods:energyPeriods.length?'extracted':'unreliable',energy_price_components:energyPriceComplete?'extracted':'unreliable',power_periods:row.powerDetail?.reliable&&powerPeriods.length?'extracted':'unreliable',power_price_components:powerPriceComplete?'extracted':'unreliable',maximeters:mx._reliable?'extracted':'unreliable',excess_detail:Number(row.excess)?'extracted':'not_present',reactive_detail:Number(row.reactive)?'extracted':'not_present',reactive_measurements:reactivePeriods.length?'extracted':'not_present',capacitive_measurements:Object.keys(capacitiveMeasurements).length?'extracted':'not_present',compensation:Number(row.compensation)?'extracted':'not_present',social_bonus:Number(row.social)?'extracted':'not_present',meter_rental:Number(row.rental)?'extracted':'not_present',electricity_tax:Number(row.tax)?'extracted':'not_present',tax_lines:taxLines.length?'extracted':'not_present',distributor_rights:'not_present',integrator_adjustment:'not_present',reactive_regularization:'not_present'};
const assessment=row.readOk&&energyPeriods.length&&powerPeriods.length?'complete':'needs_review';
return{file:file.name,invoiceNumber:row.invoiceNumber,cups:row.cups,tariff,periodText,period,total:Number(row.total)||0,kwh:Number(row.kwh)||0,energy:Number(row.energy)||0,power:Number(row.power)||0,excess:Number(row.excess)||0,reactive:Number(row.reactive)||0,compensation:Number(row.compensation)||0,social:Number(row.social)||0,rental:Number(row.rental)||0,tax:Number(row.tax)||0,vat:Number(row.vat)||0,igic:Number(row.igic)||0,distributorCharges:Number(row.distributorCharges)||0,other:Number(row.other)||0,accounted:Number(row.accounted)||0,diff:Number(row.diff)||0,energyPeriods,powerPeriods,maximeterRows,excessPeriods:[],reactivePeriods:[],taxLines,adjustments,distributorRights:[],distributor:row.distributor||'',retailer:row.retailer||'Repsol Comercializadora de Electricidad y Gas, S.L.U.',contract:row.contract||row.contractNumber||'',powerReliable:!!row.readOk&&!!row.powerDetail?.reliable,holderName:row.company||'',holderTaxId:row.taxId||'',sourceSupplyAddress:row.supplyAddress||'',accessContract:row.accessContract||'',issueDate:row.issueDate||'',contractType:row.contractType||'',contractEndDate:row.renewalDate||'',meterNumber:row.meterNumber||'',readingStatus:row.readingStatus||'unknown',readingSourceLabel:row.readingSourceLabel||'',completeness,assessment};
}
function extractNaturgy(d,file){
const parser=window.IBTNaturgyParser;if(!parser?.parse)return null;
const row=parser.parse(d,file,{parserVersion:window.IBT_PARSER_VERSION||'NATURGY',readingClassifier:window.IBTReadingStatus?.classify});if(!row||row.unsupported)return null;
const periodText=norm(row.period),period=parsePeriod(periodText),tariff=norm(row.tariff),periods=row.periods||{},contracted=row.contracted||{},mx=row.maximeters||{};
const energyPeriods=Object.entries(periods).filter(([key])=>/^P[1-6]$/.test(key)).map(([key,value])=>({period:Number(key.slice(1)),consumption_kwh:value?.consumption==null?null:Number(value.consumption),energy_cost_eur:value?.cost==null?null:Number(value.cost),unit_price_eur_kwh:value?.price==null?null:Number(value.price)}));
const entries=row.powerDetail?.entries||[],powerKeys=Object.keys(contracted).filter(key=>/^P[1-6]$/.test(key)).sort((a,b)=>Number(a.slice(1))-Number(b.slice(1)));
const powerPeriods=powerKeys.map(key=>{const periodNo=Number(key.slice(1)),entry=entries.find(item=>Number(item?.period)===periodNo)||entries[periodNo-1];return{period:periodNo,contracted_kw:Number(contracted[key])||0,billed_power_eur:entry?.amount==null?null:Number(entry.amount),unit_price_eur_kw_day:entry?.price==null?null:Number(entry.price)};});
const maximeterRows=Object.keys(mx).filter(key=>/^P[1-6]$/.test(key)).map(key=>({period:Number(key.slice(1)),maximeter_kw:Number(mx[key])||0,reliable:!!mx._reliable,source:'NATURGY · maxímetro'}));
const taxLines=[];if(Number(row.tax))taxLines.push({tax_type:'ELECTRICITY_TAX',label:'Impuesto electricidad',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.tax)});if(Number(row.vat))taxLines.push({tax_type:'IVA',label:'IVA',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.vat)});if(Number(row.igic))taxLines.push({tax_type:'IGIC',label:'IGIC',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.igic)});
const adjustments=Array.isArray(row.adjustments)?row.adjustments.map(item=>({...item})):[];if(!adjustments.length&&Number(row.discounts))adjustments.push({concept:'Descuento comercial',amount_eur:Number(row.discounts),category:'discount'});
const reactiveMeasurements=row.reactivePeriods||{};
const reactivePeriods=Object.entries(reactiveMeasurements)
...
const completeness={version:COMPLETENESS_VERSION,invoice_number:status(row.invoiceNumber),cups:status(row.cups),billing_period:status(period.start&&period.end),issue_date:status(row.issueDate,false),holder_name:status(row.company),holder_tax_id:status(row.taxId,false),supply_address:status(row.supplyAddress),access_contract_number:status(row.accessContract,false),contract_number:status(row.contract,false),contract_type:status(row.contractType,false),contract_end_date:status(row.renewalDate,false),meter_number:status(row.meterNumber,false),tariff:status(tariff),distributor:status(row.distributor,false),energy_periods:energyPeriods.length?'extracted':'unreliable',energy_price_components:energyPriceComplete?'extracted':'unreliable',power_periods:row.powerDetail?.reliable&&powerPeriods.length?'extracted':'unreliable',power_price_components:powerPriceComplete?'extracted':'unreliable',maximeters:mx._reliable?'extracted':'unreliable',excess_detail:Number(row.excess)?'extracted':'not_present',reactive_detail:Number(row.reactive)?'extracted':'not_present',reactive_measurements:reactivePeriods.length?'extracted':'not_present',capacitive_measurements:Object.keys(capacitiveMeasurements).length?'extracted':'not_present',compensation:Number(row.compensation)?'extracted':'not_present',social_bonus:Number(row.social)?'extracted':'not_present',meter_rental:Number(row.rental)?'extracted':'not_present',electricity_tax:Number(row.tax)?'extracted':'not_present',tax_lines:taxLines.length?'extracted':'not_present',distributor_rights:'not_present',integrator_adjustment:'not_present',reactive_regularization:'not_present'};
const assessment=row.readOk&&energyPeriods.length&&powerPeriods.length?'complete':'needs_review';
return{file:file.name,invoiceNumber:row.invoiceNumber,cups:row.cups,tariff,periodText,period,total:Number(row.total)||0,kwh:Number(row.kwh)||0,energy:Number(row.energy)||0,power:Number(row.power)||0,excess:Number(row.excess)||0,reactive:Number(row.reactive)||0,compensation:Number(row.compensation)||0,social:Number(row.social)||0,rental:Number(row.rental)||0,tax:Number(row.tax)||0,vat:Number(row.vat)||0,igic:Number(row.igic)||0,distributorCharges:Number(row.distributorCharges)||0,other:Number(row.other)||0,accounted:Number(row.accounted)||0,diff:Number(row.diff)||0,energyPeriods,powerPeriods,maximeterRows,excessPeriods:[],reactivePeriods:[],taxLines,adjustments,distributorRights:[],distributor:row.distributor||'',retailer:row.retailer||'Naturgy Clientes, S.A.U.',contract:row.contract||row.contractNumber||'',powerReliable:!!row.readOk&&!!row.powerDetail?.reliable,holderName:row.company||'',holderTaxId:row.taxId||'',sourceSupplyAddress:row.supplyAddress||'',accessContract:row.accessContract||'',issueDate:row.issueDate||'',contractType:row.contractType||'',contractEndDate:row.renewalDate||'',meterNumber:row.meterNumber||'',readingStatus:row.readingStatus||'unknown',readingSourceLabel:row.readingSourceLabel||'',completeness,assessment};
}
function extractUenergia(d,file){
const parser=window.IBTUenergiaParser;if(!parser?.parse)return null;
const row=parser.parse(d,file,{parserVersion:window.IBT_PARSER_VERSION||'UENERGIA',readingClassifier:window.IBTReadingStatus?.classify});if(!row||row.unsupported)return null;
const periodText=norm(row.period),period=parsePeriod(periodText),tariff=norm(row.tariff),periods=row.periods||{},contracted=row.contracted||{},mx=row.maximeters||{};
const energyPeriods=Object.entries(periods).filter(([key])=>/^P[1-6]$/.test(key)).map(([key,value])=>({period:Number(key.slice(1)),consumption_kwh:value?.consumption==null?null:Number(value.consumption),energy_cost_eur:value?.cost==null?null:Number(value.cost),unit_price_eur_kwh:value?.price==null?null:Number(value.price)}));
const entries=row.powerDetail?.entries||[],powerKeys=Object.keys(contracted).filter(key=>/^P[1-6]$/.test(key)).sort((a,b)=>Number(a.slice(1))-Number(b.slice(1)));
const powerPeriods=powerKeys.map(key=>{const periodNo=Number(key.slice(1)),entry=entries.find(item=>Number(item?.period)===periodNo)||entries[periodNo-1];return{period:periodNo,contracted_kw:Number(contracted[key])||0,billed_power_eur:entry?.amount==null?null:Number(entry.amount),unit_price_eur_kw_day:entry?.price==null?null:Number(entry.price)};});
const maximeterRows=Object.keys(mx).filter(key=>/^P[1-6]$/.test(key)).map(key=>({period:Number(key.slice(1)),maximeter_kw:Number(mx[key])||0,reliable:!!mx._reliable,source:'U Energía · POTENCIA / maxímetro'}));
const taxLines=[];if(Number(row.tax))taxLines.push({tax_type:'ELECTRICITY_TAX',label:'Impuesto sobre la electricidad',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.tax)});if(Number(row.vat))taxLines.push({tax_type:'IVA',label:'IVA',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.vat)});if(Number(row.igic))taxLines.push({tax_type:'IGIC',label:'IGIC',rate_pct:null,taxable_base_eur:null,amount_eur:Number(row.igic)});
const adjustments=Array.isArray(row.adjustments)?row.adjustments.map(item=>({...item})):[];
const energyPriceComplete=energyPeriods.length>0&&energyPeriods.every(item=>item.consumption_kwh===0||item.unit_price_eur_kwh!=null),powerPriceComplete=powerPeriods.length>0&&powerPeriods.every(item=>item.unit_price_eur_kw_day!=null);
const completeness={version:COMPLETENESS_VERSION,invoice_number:status(row.invoiceNumber),cups:status(row.cups),billing_period:status(period.start&&period.end),issue_date:status(row.issueDate,false),holder_name:status(row.company),holder_tax_id:status(row.taxId,false),supply_address:status(row.supplyAddress),access_contract_number:status(row.accessContract,false),contract_number:status(row.contract,false),contract_type:status(row.contractType,false),contract_end_date:status(row.renewalDate,false),meter_number:status(row.meterNumber,false),tariff:status(tariff),distributor:status(row.distributor,false),energy_periods:energyPeriods.length?'extracted':'unreliable',energy_price_components:energyPriceComplete?'extracted':'unreliable',power_periods:row.powerDetail?.reliable&&powerPeriods.length?'extracted':'unreliable',power_price_components:powerPriceComplete?'extracted':'unreliable',maximeters:mx._reliable?'extracted':'unreliable',excess_detail:Number(row.excess)?'extracted':'not_present',reactive_detail:Number(row.reactive)?'extracted':'not_present',compensation:Number(row.compensation)?'extracted':'not_present',social_bonus:Number(row.social)?'extracted':'not_present',meter_rental:Number(row.rental)?'extracted':'not_present',electricity_tax:Number(row.tax)?'extracted':'not_present',tax_lines:taxLines.length?'extracted':'not_present',distributor_rights:'not_present',integrator_adjustment:'not_present',reactive_regularization:'not_present'};
const assessment=row.readOk&&energyPeriods.length&&powerPeriods.length?'complete':'needs_review';
return{file:file.name,invoiceNumber:row.invoiceNumber,cups:row.cups,tariff,periodText,period,total:Number(row.total)||0,kwh:Number(row.kwh)||0,energy:Number(row.energy)||0,power:Number(row.power)||0,excess:Number(row.excess)||0,reactive:Number(row.reactive)||0,compensation:Number(row.compensation)||0,social:Number(row.social)||0,rental:Number(row.rental)||0,tax:Number(row.tax)||0,vat:Number(row.vat)||0,igic:Number(row.igic)||0,distributorCharges:Number(row.distributorCharges)||0,other:Number(row.other)||0,accounted:Number(row.accounted)||0,diff:Number(row.diff)||0,energyPeriods,powerPeriods,maximeterRows,excessPeriods:[],reactivePeriods:[],taxLines,adjustments,distributorRights:[],distributor:row.distributor||'',retailer:row.retailer||'ELECTRICA SOLLERENSE SAU',contract:row.contract||row.contractNumber||'',powerReliable:!!row.readOk&&!!row.powerDetail?.reliable,holderName:row.company||'',holderTaxId:row.taxId||'',sourceSupplyAddress:row.supplyAddress||'',accessContract:row.accessContract||'',issueDate:row.issueDate||'',contractType:row.contractType||'',contractEndDate:row.renewalDate||'',meterNumber:row.meterNumber||'',readingStatus:row.readingStatus||'unknown',readingSourceLabel:row.readingSourceLabel||'',completeness,assessment};
}
function extractHistory(d,file){const uenergia=window.IBTUenergiaParser;if(uenergia?.detect?.(d))return extractUenergia(d,file);const iberdrola=window.IBTIberdrolaParser;if(iberdrola?.detect?.(d))return extractIberdrola(d,file);const repsol=window.IBTRepsolParser;if(repsol?.detect?.(d))return extractRepsol(d,file);const naturgy=window.IBTNaturgyParser;if(naturgy?.detect?.(d))return extractNaturgy(d,file);const formats=window.IBTInvoiceFormats,format=formats?.detect?.(d.text);if(format==='endesa')return extractEndesa(d,file);if(format==='fenie')return extractFenie(d,file);return null;}
...
const source=await readPdf(file),uenergia=window.IBTUenergiaParser,iberdrola=window.IBTIberdrolaParser,repsol=window.IBTRepsolParser,naturgy=window.IBTNaturgyParser,fallback=window.IBTFenieOcrFallback;let prepared;if(uenergia?.detect?.(source))prepared={data:source,attempted:false,error:null};else prepared=(iberdrola?.detect?.(source)||repsol?.detect?.(source)||naturgy?.detect?.(source))?{data:source,attempted:false,error:null}:fallback?.prepare?await fallback.prepare(file,source,pdfjsLib):{data:source,attempted:false,error:null};
